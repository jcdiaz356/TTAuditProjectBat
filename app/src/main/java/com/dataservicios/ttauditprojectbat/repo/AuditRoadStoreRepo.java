...
//            wishList = helper.getAuditRoadStoreDao().queryBuilder().where().eq("store_id",store_id).query();
    ...
    public List<AuditRoadStore> findByStoreAndRoadId(int store_id,int road_id) {

        List<AuditRoadStore> wishList = null;
        try {
            wishList = helper.getAuditRoadStoreDao().queryBuilder().orderBy("order",true)
                    .where().eq("store_id",store_id)
                    .and().eq("road_id",road_id).query();
//            wishList = helper.getAuditRoadStoreDao().queryBuilder().where().eq("store_id",store_id).query();
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public Object findByStoreIdAndAuditIdAndRouteId(int store_id, int audit_id,int route_id) {

        AuditRoadStore wishList = null;
        try {
            wishList =  helper.getAuditRoadStoreDao().queryBuilder().where()
                    .eq("store_id",store_id).and()
                    .eq("audit_id",audit_id).and()
                    .eq("road_id",route_id)
                    .queryForFirst();
        } catch (SQLException e) {
            e.printStackTrace();
